...
/** T1-08: Maximum recursion depth for flattenArgs */
const MAX_FLATTEN_DEPTH = 50

/**
 * Flattens nested arrays (from range references) into a single array.
 * Depth-limited to prevent stack overflow from deeply nested structures.
 */
function flattenArgs(args: unknown[], depth: number = 0): unknown[] {
  if (depth > MAX_FLATTEN_DEPTH) {
    throw new FormulaError(
      `flattenArgs exceeds maximum recursion depth of ${MAX_FLATTEN_DEPTH}. Data structure is too deeply nested.`
    )
  }
  ...
  for (const arg of args) {
    if (Array.isArray(arg)) {
      result.push(...flattenArgs(arg, depth + 1))
    } else {
      result.push(arg)
    }
  }
