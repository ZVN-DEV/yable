...
export function resolveRowSpans<TData extends RowData>(
  ...
  for (const colDef of columnDefs) {
    // Access rowSpan via the column def extensions (it's a custom extension, not in ColumnDefExtensions)
    const rowSpanFn = (colDef as ColumnDef<TData> & { rowSpan?: RowSpanFn<TData> }).rowSpan
    ...
    while (i < rows.length) {
      ...
      // T1-09: Wrap user rowSpanFn callback in try-catch
      let span: number
      try {
