...
`postSortRows` runs after the sorted row model is built and lets you reorder
the final rows before they render — the AG Grid `postSortRows` equivalent. It
receives the sorted `Row` array; return a new array or mutate the given one in
place. It runs on every sort (and even with no active sort), so it is the right
place to keep child rows grouped under their parents:
...
const table = useTable({
  ...
  columns,
  postSortRows: (rows) => {
    // Float pinned rows to the top regardless of the active column sort.
...
`postSortRows` is skipped under `manualSorting`, where the server controls order.
