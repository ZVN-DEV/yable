...
<Callout type="warn">
  Turnkey grouped/aggregated row rendering is on the roadmap. The aggregation functions
  below ship today (and power pivot value fields via `getPivotRowModel()`), but rendering
  grouped rows and their aggregated cells through `<Table>` is not yet wired.
</Callout>
