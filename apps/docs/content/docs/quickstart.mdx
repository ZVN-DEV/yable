...
description: Get from zero to a fully interactive data table in 11 steps.
---
...
Sorting is enabled by default — all columns are sortable out of the box. If you only want *some* columns to be sortable, disable it on the ones you don't want:
...
```tsx
import { mergeEditChanges } from '@zvndev/yable-react'
...
export function EmployeeTable() {
  ...
  const table = useTable({
    ...
    onEditCommit: (changes) => {
      setData((prev) => mergeEditChanges(prev, changes))
    },
  })
...
Cells automatically show pending spinners during saves, error badges on failure (with retry on click), and conflict indicators if the server value changed underneath. See the [Async Commits Guide](/docs/features/async-commits) and [Feature Documentation](/docs/features#async-cell-commits) for the full API.
...
```tsx
import { useState } from 'react'
import { createColumnHelper, useTable, Table, Pagination, GlobalFilter, mergeEditChanges } from '@zvndev/yable-react'
import type { SortingState } from '@zvndev/yable-react'
import '@zvndev/yable-themes'
...
- Read the [Feature Documentation](/docs/features) for detailed guides on every feature (column pinning, row selection, grouping, tree data, formulas, pivot tables, etc.)
- Browse the [API Reference](/docs/api) for the complete list of table options, methods, and types
- Check the [Contributing Guide](https://github.com/zvndev/yable/blob/main/CONTRIBUTING.md) if you want to help build Yable
